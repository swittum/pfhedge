...
class JaxLayer(torch.nn.Module):
    def __init__(self,circuit):
        super().__init__()
        batched_func = jax.jit(jnp.vectorize(circuit.qnode,excluded=[1],signature='(n)->(m)'))
        self.weights = Parameter(torch.empty(circuit.weight_shape))
        torch.nn.init.uniform(self.weights,a=0,b=2*np.pi)
        def calc_grad(x,grad,weights):
            y, vjp = jax.vjp(circuit.qnode,x,weights)
            return vjp(grad)
        batched_calc_grad = jax.jit(jnp.vectorize(calc_grad, excluded=[2], signature='(n),(k)->(n),(l,n)'))
